...
export var fire_probability = 0.6

onready var EnemyLaser = load("res://Scenes/Enemy Laser.tscn")
...
func _on_Timer_timeout():
	...
	if randf() < fire_probability:
		var b = EnemyLaser.instance()
		b.position = position
		b.position.y += 25
		get_node("/root/Game/Enemy Lasers").add_child(b)
